...
export class CoreConfig {
  ...
  @IsNotEmpty({
    message:
      'Set Env variable POSTGRESQL_URL, example: postgres://localhost:27017/my-app-local-db',
  })
  postgresURL: string;
  ...
  constructor(private configService: ConfigService<any, true>) {
    // Инициализируем все свойства в конструкторе
    this.port = Number(this.configService.get('PORT'));
    this.mongoURL = this.configService.get('MONGO_URL');
    this.postgresURL = this.configService.get('POSTGRESQL_URL');
